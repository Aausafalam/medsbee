                ...
                <div className="relative max-w-7xl mx-auto px-0 md:px-10">
                    ...
                    >
                        {pricingData.plans.map((plan) => (
                            <div
                                key={plan.id}
                                className={`flex-shrink-0 w-[85vw] sm:w-[350px] py-2 md:py-3  md:w-auto snap-center relative rounded-[1.5rem] md:rounded-[2rem] overflow-hidden transition-all duration-500 flex flex-col h-auto shadow-lg hover:shadow-xl ${plan.isFeatured
                                    ? "bg-[#002B5B] text-white ring-2 ring-secondary/30 z-10 max-md:order-first"
                                    : "bg-white text-primary border border-blue-100/50"
                                    }`}
                            >
                                {/* Star Badge */}
                                <div className="absolute top-5 left-1/2 -translate-x-1/2 flex flex-col items-center gap-0.5">
                                    ...
                                    {plan.badge && (
                                        <span className="text-[10px] mt-1 font-bold text-secondary uppercase tracking-tighter whitespace-nowrap leading-none">
                                            {plan.badge}
                                        </span>
                                    )}
                                </div>
                                ...
                                <div className="p-6 mt-2 md:p-8 pt-10 md:pt-14 space-y-4 md:space-y-6 flex-grow flex flex-col">
                                    ...
                                    <div className="text-center space-y-2">
                                        ...
                                        <p className={`text-[13px] font-medium leading-tight px-2 ${plan.isFeatured ? "text-blue-100/70" : "text-gray-500"}`}>
                                            {plan.description}
                                        </p>
                                    ...
                                    <ul className="space-y-2.5 md:space-y-3 flex-grow py-3 border-y border-dashed border-primary/10">
                                        {plan.features.map((feature, idx) => (
                                            <li key={idx} className={`flex items-start gap-2.5 transition-opacity duration-300 ${!feature.included ? "opacity-50" : ""}`}>
                                                <div className={`mt-0.5 flex-shrink-0 ${feature.included
                                                    ? "text-green-500"
                                                    : "text-red-400"
                                                    }`}>
                                                    {feature.included ? (
                                                ...
                                                <span className={`text-[13px] md:text-sm leading-tight font-medium ${plan.isFeatured ? "text-white" : "text-gray-700"
